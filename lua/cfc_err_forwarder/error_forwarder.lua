...
do
  ...
  local _base_0 = {
    ...
    queueError = function(self, isRuntime, fullError, sourceFile, sourceLine, errorString, stack, ply)
      ...
      return rawset(self.queue, fullError, newError)
    end,
    unqueueError = function(self, fullError)
      local thisErr = rawget(self.queue, fullError)
      if thisErr then
        for k in pairs(thisErr) do
          rawset(thisErr, k, nil)
        end
      end
      return rawset(self.queue, fullError, nil)
    end,
    incrementError = function(self, fullError)
      local thisErr = rawget(self.queue, fullError)
      local count = rawget(thisErr, "count")
      rawset(thisErr, "count", count + 1)
      return rawset(thisErr, "occurredAt", osTime())
    end,
